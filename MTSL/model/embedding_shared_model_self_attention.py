...
class EmbeddingSharedModelAttention(nn.Module):
    ...
    def _get_rnn_output(self, input_word, input_char, main_task, mask, hx=None):
        ...
        lstm_out, hn = utils.recover_rnn_seq(seq_output, rev_order, hx=hn, batch_first=True)
        lstm_out = self.dropout_out(lstm_out)
        attn_out, _ = self.mutihead_attention(lstm_out, lstm_out, lstm_out, None)
        attn_out = 0.9 * lstm_out + 0.1 * attn_out
        if self.use_lm:
            output_size = attn_out.size()
            # print output_size
            lm = attn_out.view(output_size[0], output_size[1], 2, -1)
            # print output_lm.size()
            lm_fw = lm[:, :, 0]
            lm_bw = lm[:, :, 1]
            return attn_out, hn, mask, length, lm_fw, lm_bw
        else:
            return attn_out, hn, mask, length
